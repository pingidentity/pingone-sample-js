...
The default OAuth 2.0 flow illustrated here is an authorization code grant type. But for a demonstration purposes we provided a way to test `implicit` and `client credentials` types by proper adjusting [config.cfg](config.cfg) file. 
